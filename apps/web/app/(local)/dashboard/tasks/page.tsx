...
export default function TasksPage() {
  ...
  const { stats, mutate: mutateStats } = useTaskStats();
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <Box sx={{ flex: 1 }}>
        {isLoading ? (
          ...
          <Paper sx={{ p: 4, textAlign: "center" }}>
            <Typography color="error">Có lỗi xảy ra: {error.message}</Typography>
          </Paper>
        ) : tasks.length === 0 ? (
          <Paper sx={{ p: 4, textAlign: "center" }}>
          ...
          <Box>
            {viewMode === "grid" ? (
              <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 2 }}>
                {tasks.map((task: Task) => (
                  <Paper key={task._id} sx={{ p: 2, border: "1px solid", borderColor: "divider" }}>
